----------------------------------------------------------------------------------
-- Módulo: top_level - Versión Definitiva (Fail-Safe NC + Fix Velocidad)
----------------------------------------------------------------------------------
...
entity top_level is
    Generic (
        ...
        SYS_MOT_FREQ : integer := 1000       -- ¡Velocidad segura para arrancar!
    );
    Port ( 
        clk       : in  STD_LOGIC;
        reset     : in  STD_LOGIC;
...
architecture Structural of top_level is
    ...
    -- Cables internos de puente para interceptar la señal antes del motor
    signal w_step_x_req, w_step_y_req : STD_LOGIC;
    signal w_dir_x_int, w_dir_y_int   : STD_LOGIC;

    -- SEÑALES UNIFICADAS DE SEGURIDAD
    signal w_crash_x, w_crash_y       : STD_LOGIC;

begin

    -- ==========================================
    -- LÓGICA DE SEGURIDAD (INTERRUPTORES NC)
    -- ==========================================
    -- Al ser Normalmente Cerrados, dan un '1' en reposo y un '0' al chocar.
    -- Invertimos la señal: Si lee un '0' físico (not limit), se activa el Crash ('1').
    w_crash_x <= limit_x;
    w_crash_y <= limit_y;

    -- Freno de emergencia lógico si cualquier eje choca
    w_abort_motion <= w_crash_x or w_crash_y;
    ...
    Inst_Bresenham: bresenham_2d
        port map (
            ...
            start_motion => w_start_mot, abort_motion => w_abort_motion, -- Usa la señal unificada
            steps_x => w_steps_x, steps_y => w_steps_y,
    ...
    -- ==========================================
    -- DRIVERS TB6600 FÍSICOS (Pulsos de 5 microsegundos)
    -- ==========================================
    Inst_Driver_X: tb6600_axis_driver
        ...
        port map (
            ...
            enable_request => '1', limit_active => w_crash_x,  -- Usa la señal unificada
    ...
    Inst_Driver_Y: tb6600_axis_driver
        ...
        port map (
            ...
            enable_request => '1', limit_active => w_crash_y,  -- Usa la señal unificada
