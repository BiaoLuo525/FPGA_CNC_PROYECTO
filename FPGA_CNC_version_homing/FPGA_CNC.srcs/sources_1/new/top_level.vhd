----------------------------------------------------------------------------------
-- Módulo: top_level - FIX: Homing, Enmascaramiento y Clean Code
----------------------------------------------------------------------------------
...
architecture Structural of top_level is
    ...
    component uart_rx
        ...
        Port ( clk, reset, rx : in STD_LOGIC;
               data_out : out STD_LOGIC_VECTOR(7 downto 0); rx_ready : out STD_LOGIC );
    end component;
    ...
    component pen_controller
        Generic ( CLK_FREQ_HZ : integer := 100000000; PWM_HZ : integer := 50;
                  PULSE_UP_US : integer := 1000; PULSE_DOWN_US : integer := 2000 );
    ...
    -- Señal Unificada Cruda de Sensor (Inversión de NC)
    signal w_crash_x, w_crash_y : STD_LOGIC;
    
    signal w_is_homing : STD_LOGIC;

    -- Señales Enmascaradas para los Drivers Físicos
    signal w_limit_x_drv, w_limit_y_drv : STD_LOGIC;

begin

    -- Inversión física centralizada
    w_crash_x <= not limit_x;
    w_crash_y <= not limit_y;

    -- ==========================================
    -- GESTIÓN DEL BLOQUEO (MODO NORMAL VS HOMING)
    -- ==========================================
    w_abort_motion <= (w_crash_x or w_crash_y) and (not w_is_homing);
    w_limit_x_drv  <= w_crash_x and (not w_is_homing);
    w_limit_y_drv  <= w_crash_y and (not w_is_homing);
    ...
    Inst_UART_TX: uart_tx
        generic map ( CLK_FREQ => SYS_CLK_FREQ, BAUD_RATE => SYS_BAUD )
        port map ( clk => clk, reset => reset, tx_start => w_tx_start,
                   data_in => w_tx_data, tx => tx );
    ...
    Inst_FSM_Main: fsm_main
        port map (
            ...
            limit_x      => w_crash_x,    limit_y      => w_crash_y,    -- UNIFICADO
    ...
    Inst_Pen_Controller: pen_controller
        generic map ( CLK_FREQ_HZ => SYS_CLK_FREQ, PWM_HZ => 50,
                      PULSE_UP_US => 1000, PULSE_DOWN_US => 2000 )
        port map (
            clk         => clk,         reset       => reset,
            update      => w_pen_update, pen_down_cmd => w_pen_state,
    ...
    Inst_Driver_X: tb6600_axis_driver
        ...
        port map (
            ...
            enable_request => '1',           limit_active   => w_limit_x_drv,
            step_out       => step_x,        dir_out        => dir_x,
            enable_out     => open,          busy           => open,
            limit_fault    => open,          dropped_step   => open
        );
    ...
    Inst_Driver_Y: tb6600_axis_driver
        ...
        port map (
            ...
            enable_request => '1',           limit_active   => w_limit_y_drv,
